function [mod_dip] = three_layer_dip(baz,phineg,phi,phipos,dtneg,dt,dtpos,phi_mod,dt_mod,modeldip,freq,mod_dt,mis_sch,dip_stp)
...
%% Loop through MS Effective Splitting

tic
hh=waitbar(0,'progress');
fontsize(hh,16,"points");

mod_dip = zeros(m1*m2, 6);
...
for i=1:m1
    for j=1:m2
        ...
        for k=1:m3
            ...
            if strcmpi(dip_stp, 'HDH')
                phiv=[phi2 phi1 phi3];
                dtv=[dt2 dt1 dt3];
            elseif strcmpi(dip_stp, 'DHH')
                phiv=[phi1 phi2 phi3];
                dtv=[dt1 dt2 dt3];
            else
                phiv=[phi2 phi3 phi1];
        ...
        mod_dip(count1,:)=[mod_hor(i,1) mod_hor(i,2) mod_hor(j,1) mod_hor(j,2) tmp_misfit misf1];
...
mod_dip = mod_dip(~all(mod_two == 0, 2), :);
mod_dip=mod_dip(~isnan(mod_dip(:,5)),:); % Remove NaN values
