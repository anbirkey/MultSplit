function [fast_eff, tlag_eff] = MS_effective_splitting_SS_vec(f, spol_vec, fast, tlag)
    ...
    % Unwind fast to (-90, 90]
    fast_eff = mod(fast_eff + 90, 180) - 90;
    
    % MSAT's flip: if tlag < 0, add 90 to fast, take abs of tlag
    flip = tlag_eff < 0;
    fast_eff(flip) = mod(fast_eff(flip) + 90 + 90, 180) - 90;
